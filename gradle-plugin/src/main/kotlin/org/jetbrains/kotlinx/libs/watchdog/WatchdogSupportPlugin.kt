...
public class WatchdogSupportPlugin : DevKitSupportPlugin(PluginInfo.PLUGIN_INFO) {
    override fun apply(target: Project) {
        val extension = target.extensions.create("libsWatchdog", WatchdogGradleExtension::class.java)
    ...
    private companion object {
        ...
        private fun tapmocSuggestion(projectPath: String): String = """
            |Project '$projectPath' applies libs-watchdog but not Tapmoc. The watchdog guards the shape of
            ...
            |Disable this suggestion with `libsWatchdog { suggestTapmoc.set(false) }`.
        """.trimMargin()
    }
}
